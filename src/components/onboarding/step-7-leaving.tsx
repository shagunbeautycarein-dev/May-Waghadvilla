...
export function Step7Leaving({ agreed, onNext, onBack }: Props) {
  const [checked, setChecked] = useState(agreed || false);
  const [content, setContent] = useState<string>("Loading...");

  useEffect(() => {
    fetch("/api/settings/leaving_policy")
      .then((res) => res.json())
      .then((data) => setContent(data.value || "Leaving policy content not set."))
      .catch(() => setContent("Failed to load leaving policy."));
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex items-start gap-3 bg-white rounded-xl border border-slate-100 p-4">
        ...
        />
        <Label htmlFor="leaving" className="text-sm font-normal leading-snug text-slate-700 cursor-pointer">
          I understand the leaving policy *
        </Label>
      </div>
